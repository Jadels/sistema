...
def listar_villas(request):
    ...
    if queryset:
        villas=Villa.objects.filter(
            Q(identificador__icontains=queryset) 
        ).distinct()
    
    
    data={
        'villas':villas,'queryset':queryset
    }
...
def listar_productobodega(request,id):
    queryset= request.GET.get("buscar")
    print(queryset)
    productos=BodegaProductos.objects.select_related('bodega','producto').filter(bodega=id).values('id','producto_id__id','producto_id__clave','producto_id__descripcion','producto_id__unidad','cantidad','minimo','bodega_id')
    if queryset:
        productos=BodegaProductos.objects.select_related('bodega','producto').filter(bodega=id).values('producto_id__id','producto_id__clave','producto_id__descripcion','producto_id__unidad','cantidad','minimo','bodega_id').filter(
            Q(producto_id__descripcion__icontains=queryset) 
        ).distinct()
        print(productos)
    ...
    if request.method =="POST":
        ...
        for cant in cantidad:
            if(cant == ''):
                ...
                descripcion=desc[x]
                datos={'solicita':solicita,'fecha':fecha,'obra':obra,'solicitud':solicitud,'cantidad':cantidad,'bodegaproducto':bodegaproducto,'descripcion':descripcion}
                print(datos)
...
def ver_compra(request,solicitud):
    ...
    solicitudes=Solicitud.objects.select_related('bodegaproducto','compra').values('bodegaproducto_id','solicitud','cantidad','descripcion','bodegaproducto_id__cantidad', 'compra','id','fecha','solicita').order_by('solicitud').filter(solicitud=solicitud)  
...
def ver_recepcion(request,solicitud):
    solicitudes=Solicitud.objects.select_related('bodegaproducto','compra','recepcion').values('bodegaproducto_id','cantidad','solicitud','bodegaproducto_id__cantidad','descripcion','id','solicita','compra','recepcion__llegada','recepcion__pendiente','recepcion__utilizado','recepcion__saldo').filter(solicitud=solicitud)
    id=Solicitud.objects.select_related('bodegaproducto','compra','recepcion').values('id').filter(solicitud=solicitud)    
    compra=Compra.objects.select_related('solicitud').filter(solicitud_id__in=id).values() 
    data={
        'solicitudes':solicitudes,     
...
def recepcion_registro(request,solicitud):
    solicitudes=Solicitud.objects.select_related('bodegaproducto','compra').values('bodegaproducto_id','solicitud','cantidad','descripcion','bodegaproducto_id__cantidad','compra','id').annotate(total=Count('solicitud')).order_by('solicitud','total').filter(solicitud=solicitud)
    id=Solicitud.objects.select_related('bodegaproducto','compra','recepcion').values('id').filter(solicitud=solicitud)    
    compra=Compra.objects.select_related('solicitud').filter(solicitud_id__in=id).values() 
    data={
        'solicitudes':solicitudes,
